#!/usr/bin/env python3
import click, json, os, sys, shutil, time, hashlib, sqlite3, subprocess, pathlib, datetime, textwrap
...
def ensure_scaffold(project_root: Path):
    for p in ["00-admin/index", "00-admin/logs", "01-sources", "02-working", "03-outputs", "99-archive", "templates/prompts", "templates/docs", "scripts"]:
...
def reset(ctx, apply):
    ...
    ensure_scaffold(pr)
    (pr / "00-admin" / "version.json").write_text(json.dumps({"version": "0.1", "updated": datetime.date.today().isoformat()}, indent=2))
    (pr / "CHANGELOG.md").write_text("# CHANGELOG\n\n- v0.1 Initial scaffold\n")
    click.echo("Reset complete (non-destructive).")

@cli.command()
@click.option("--capstone", "caps", multiple=True, type=click.Path(path_type=Path), required=True)
...
def master(ctx, template):
    pr = ctx.obj["project_root"]
    ensure_scaffold(pr)
    out = pr / "03-outputs" / f"Housing_Subsidy_Reform_MASTER_v0.1_{datetime.date.today().strftime('%Y%m%d')}.docx"
