...
2. From inside your extension directory (for example `raycast-extension/`):
   ```bash
   ray run .
   ```
...
2. Add `"typeRoots": ["./node_modules/@types"]` to `tsconfig.json` to avoid duplicate React definitions.
...
2. In the project root, enable APA 7 templates:
   ```bash
   quarto add wjschne/apaquarto
   ```
3. Confirm that Quarto sees the APA format with `quarto check`. Capture the output in `docs/logs/quarto-install.md`.
4. Update `docs/policy_brief_apa7.qmd` and the bill drafting shell template with project metadata (title, authors, date, and contact email). Track any custom filters or CSL files alongside the QMD files.
...
2. Validate commands load without errors; note any missing environment variables in `docs/logs/raycast-install.md`.
3. Set up a minimal smoke test command (e.g., `hello-world`) to confirm Raycast can call the backend configured via `assistant_base_url`.

## 5) Shortcuts Automation

1. Create a `shortcuts/` folder in this repository to store exported `.shortcut` files and usage notes.
2. Build the following Apple Shortcuts following the instructions in `shortcuts/`:
...
3. Export each shortcut to `shortcuts/` and document triggers and required permissions.
