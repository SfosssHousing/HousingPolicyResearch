...
scenarios = ["Status_Quo_Baseline", "PET_Conservative", "PET_Base_Case", "PET_Optimistic"]
...
total_property_appreciation_conservative = year_15_base["property_appreciation_conservative"] * 10000
total_property_appreciation_optimistic = year_15_base["property_appreciation_optimistic"] * 10000
...
print(f"  Conservative (2%): ${year_15_base['property_appreciation_conservative']:,.0f}")
...
print(f"  Conservative: ${total_tenant_wealth + total_property_appreciation_conservative:,.0f}")
print(f"  Base case: ${total_tenant_wealth + total_property_appreciation_base:,.0f}")
print(f"  Optimistic: ${total_tenant_wealth + total_property_appreciation_optimistic:,.0f}")
print(f"\n--- COMPARATIVE ROI ---")
print(f"Baseline (status quo): -${cumulative_subsidy_15yr:,.0f} (sunk cost; zero returns)")
print(f"PET model net benefit (base case): +${total_tenant_wealth + total_property_appreciation_base - cumulative_subsidy_15yr:,.0f}")
