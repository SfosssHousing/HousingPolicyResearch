...
class TCAPTaskManager:
    ...
    def infer_dependencies(self):
        ...
        section_order = {
            "Intake/Triage": 0, "Research": 1, "Drafting": 2, "Decision": 3,
            "QA": 4, "Automation": 5, "Version Control": 6, "Deliverables": 7
        }
    ...
    def generate_risk_report(self):
        ...
        for task in self.tasks:
            ...
            multiplier = {"Not Started": 3, "In Progress": 2, "Complete": 0}.get(status, 2)
            
            if any(kw in task_text for kw in ["legal", "title", "aida", "504", "procurement"]):
                risk_scores["legal"] += multiplier
            if any(kw in task_text for kw in ["budget", "mou", "cfo", "funding", "roi"]):
            ...
            for task in self.tasks:
                ...
                if due_str and status != 'Complete':
                    try:
                        ...
                        if due < today:
                            overdue.append((task.get('Task', ''), due_str, (today - due).days))
                    except:
                        pass
            
            if overdue:
                f.write(f"\nOVERDUE TASKS ({len(overdue)}):\n")
                for task_name, due_date, days_over in sorted(overdue, key=lambda x: x[2], reverse=True):
...
def main():
    ...
    parser.add_argument('action', choices=['update', 'report', 'risk', 'dependencies'],
                        help='Action to perform')
    parser.add_argument('--csv', default='NYC_Housing_Subsidy_Ops_Tasks_Notion.csv',
