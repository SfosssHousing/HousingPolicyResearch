...
1. The last argument is always a callback
1. The callback can accept any number of arguments
1. The first argument passed to the callback will be treated as an error result, if the argument is truthy
1. Any number of result arguments can be passed after the "error" argument
1. The callback is called once and exactly once, either on the same tick or later tick of the JavaScript event loop.
