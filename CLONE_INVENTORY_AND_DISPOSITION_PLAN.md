...
2. Copy entire `Housing Policy Workspace/` to `/Desktop/HousingPolicyResearch_Archive/Housing_Policy_Workspace_BACKUP/`
3. Copy entire `workspace/housing_policy_scaffold/` to `/Desktop/HousingPolicyResearch_Archive/raycast_scaffold_BACKUP/`
4. Copy all ZIPs to backup folder
...
1. From `HousingPolicyResearch/artifacts/`:
   ...
   - Keep images (pet_deck_*.png)
2. From `workspace/housing_policy_scaffold/raycast-extension/`:
   - Extract `src/commands/`, `src/utils/`, `tsconfig.json`, `package.json`
   - Ensure no duplication with main repo's `raycast-extension/`
3. Section docs (Section 1, 2, 3... at root):
...
2. Delete `HousingPolicyResearch-1/` folder (after backup)
3. Delete `Housing Policy Workspace/` folder (empty)
4. Delete `workspace/housing_policy_scaffold/node_modules/` and ZIPs
5. Remove root-level duplicate " 2" files
6. Remove root-level duplicate `Section*` folders (after moving to docs/)
...
2. **node_modules Bloat**: 73M of raycast node_modules should NEVER be in version control. Will be discarded.

3. **Section Docs**: Currently named with special characters (e.g., "Section 1 – ..."). Will rename to kebab-case during move.

4. **Stash**: `stash@{0}` still contains node_modules/scaffold files from earlier reset. Can be dropped safely after cleanup.

5. **Raycast Manifest**: Main repo has `raycast.manifest.json` and `raycast-extension/`. Verify no conflicts before merging additional raycast source.
