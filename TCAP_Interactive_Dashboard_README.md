...
2. **Visualizes Dependencies** — Maps task blocking relationships across 8 project phases
3. **Tracks Risk Radar** — Real-time heat map of legal/fiscal/operational risks
4. **Maintains Version History** — Captures snapshots with timestamps and audit trails
5. **Generates Automated Reports** — Status, risk, overdue task alerts
6. **Enables Scheduling** — Cron-based daily/weekly/monthly automation
...
3. Sync results back to Notion via API
...
2. **Run baseline report**: `python3 tcap_task_automation.py report`
3. **Review risk escalations** in `TCAP_Version_History_Audit_Log.txt`
4. **Assign TASK_001 & TASK_004** to unblock dependent phases
5. **Set up cron** (see `tcap_cron_setup.txt`)
6. **Update CSV weekly** as work progresses
7. **Archive versions** using ops manual naming convention
